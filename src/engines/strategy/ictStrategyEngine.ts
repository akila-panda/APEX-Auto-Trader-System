...
export function runStrategyEngine(input: StrategyEngineInput): StrategyEngineResult {
  ...
  if (bias === 'neutral') {
    console.debug('[Strategy] tf=%s — macro neutral, skipping all detectors.', r.tfId)
    return {
      setup: null,
  ...
  // [FIX 5] Debug log — fires every cycle showing each setup's result and rejection reasons.
  // Console only — no feed UI changes.
  
  console.debug(
    '[Strategy] tf=%s bias=%s | %s',
    r.tfId,
    bias,
    Object.entries(allSetups)
      .map(([k, v]) => `${k}: valid=${v.valid} rejected=[${v.rejections.join(' / ')}]`)
      .join(' | '),
  )
